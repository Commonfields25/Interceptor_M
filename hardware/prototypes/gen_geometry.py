...
def load_parameters(line: str) -> dict:
    ...
    for path in candidates:
        if path.is_file():
            ...
            print(
                f"[gen_geometry] Loaded PARAMETERS.json from {path}  "
...
def generate_actuator_mount_geometry(params: dict) -> dict:
    ...
    pockets = {
        "ESC": {
            "centre_mm": [
                round(esc_cx + esc[0] / 2, 3),
                round(esc_cy + esc[1] / 2, 3),
                round(T / 2, 3),
            ],
            "size_mm": list(esc),
            "bounds_mm": [esc_cx, esc_cx + esc[0], esc_cy, esc_cy + esc[1], 0.0, T],
        },
        "FC": {
            "centre_mm": [
                round(fc_cx + fc[0] / 2, 3),
                round(fc_cy + fc[1] / 2, 3),
                round(T / 2, 3),
            ],
            "size_mm": list(fc),
            "bounds_mm": [fc_cx, fc_cx + fc[0], fc_cy, fc_cy + fc[1], 0.0, T],
        },
        "Battery_slot": {
            "centre_mm": [round(L / 2, 3), round(W / 2, 3), round(T, 3)],
            "width_mm": bat[0],
            "depth_mm": bat[1],
        },
    }
...
def generate_nose_cone_ring_geometry(params: dict) -> dict:
    ...
    m3_hole_ctrs = np.array(
        [[or_z, hole_r * math.cos(a), hole_r * math.sin(a)] for a in hole_angles]
    ).tolist()
...
def main():
    ...
    print(
        f"  Est. masses (g): {[(p['part_id'], p['mass_g_estimated']) for p in output['parts']]}"
    )
    ...
    for part in output["parts"]:
        ...
        assert part["mass_g_estimated"] > 0, f"Zero/negative mass for {part['part_id']}"
