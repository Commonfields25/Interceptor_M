...
def add_rect(msp, x, y, w, h, dxfattribs=None):
    msp.add_lwpolyline([(x, y), (x+w, y), (x+w, y+h), (x, y+h), (x, y)], dxfattribs=dxfattribs)
...
def create_dd400_assembly(filename):
    ...
    for i, x in enumerate(compartments):
        ...
        msp.add_text(labels[i], dxfattribs={'height': 5}).set_placement(((prev_x + x)/2, -5))
        prev_x = x
    msp.add_text(labels[-1], dxfattribs={'height': 5}).set_placement(((prev_x + L)/2, -5))
...
def create_dd400_wing_mechanism(filename):
    ...
    msp.add_lwpolyline([(0, 0), (75, 0), (75, 4), (0, 4), (0, 0)], dxfattribs={'color': 8, 'linetype': 'DASHED'})
    doc.saveas(filename)
...
def create_generic_assembly(filename, L, D, label):
    ...
    msp.add_line((L_nose, -R), (L, -R))
    msp.add_line((L, R), (L, -R))
    msp.add_text(f"{label} ASSEMBLY", dxfattribs={'height': 10}).set_placement((L/4, 50))
    doc.saveas(filename)
