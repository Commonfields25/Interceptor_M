...
def main():
    ...
    if not required_files:
        print(
            f"🔴 Violation: No validation package found for {gate_label} in {gate_pkg_dir}"
        )
        sys.exit(1)
    ...
    if gate_label in AUTO_APPROVABLE_GATES:
        if check_technical_validation(gate_id):
            print(
                f"🌟 AUTO-APPROVAL TRIGGERED: {gate_label} meets all technical criteria."
            )
            print(
                "Human DG (Director General) signature bypassed based on ISO 9001:2015 Auto-Delegation Policy."
            )
            sys.exit(0)
        else:
            print(
                f"ℹ️ {gate_label} is eligible for auto-approval but technical validation is incomplete."
            )
