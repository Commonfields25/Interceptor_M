...
def sync():
    ...
    if os.path.exists(dd_params_path):
        ...
        dd_params["segments"]["fuselage"]["L_mm"] = dd_specs["segments"]["fuselage"][
            "L_mm"
        ]
    ...
    if os.path.exists(constants_path):
        ...
        for line in lines:
            if "MASSE_INTERCEPTOR_KG =" in line:
                new_lines.append(
                    f"MASSE_INTERCEPTOR_KG = {dd_specs['mtow_g'] / 1000.0:.3f}  # Synced from PARAMETERS.json\n"
                )
            elif "LONGUEUR_INTERCEPTOR_MM =" in line:
                new_lines.append(
                    f"LONGUEUR_INTERCEPTOR_MM = {dd_specs['segments']['fuselage']['L_mm']}  # Synced from PARAMETERS.json\n"
                )
            else:
