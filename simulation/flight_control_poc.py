...
class GuidanceSystem:
    def __init__(self, mode="PN", latency_steps=0):
        ...
        self.kf_az, self.kf_el = LOSKalmanFilter(dt=_DT), LOSKalmanFilter(dt=_DT)
        self.prev_vel_c, self.measurement_buffer = None, []
    ...
    def compute_guidance(self, etat_i, etat_c):
        ...
        dist, rel_vel = math.sqrt(dist_sq), vel_c - vel_i
        az_true, el_true = math.atan2(rel_pos[1], rel_pos[0]), math.asin(rel_pos[2] / dist)
        az_noisy, el_noisy = az_true + np.random.normal(0, 0.002), el_true + np.random.normal(0, 0.002)
        ...
        if not self.initialized:
            self.kf_az.x, self.kf_el.x, self.initialized, self.prev_vel_c = np.array([az_meas, 0, 0]), np.array([el_meas, 0, 0]), True, vel_c.copy()
        self.kf_az.predict(); self.kf_el.predict(); self.kf_az.update(az_meas); self.kf_el.update(el_meas)
        omega_az, omega_el, v_clos = self.kf_az.get_rate(), self.kf_el.get_rate(), -np.dot(rel_pos, rel_vel) / dist
        accel_az, accel_el = _GAIN_PN * v_clos * omega_az * math.cos(el_true), _GAIN_PN * v_clos * omega_el
        if self.mode == "APN":
            at = (vel_c - self.prev_vel_c) / _DT
            at_perp = at - np.dot(at, rel_pos/dist) * (rel_pos/dist)
            accel_el += 0.5 * _GAIN_PN * at_perp[2]
            ub_xy = np.array([-math.sin(az_true), math.cos(az_true), 0.0])
            accel_az += 0.5 * _GAIN_PN * np.dot(at_perp, ub_xy)
