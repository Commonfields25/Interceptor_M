...
def fetch_github_issues():
    ...
    if response.status_code != 200:
        print(
            f"Erreur lors de la récupération des issues GitHub: {response.status_code}"
        )
        return []
...
def create_linear_issue(title, description, agent_id=None):
    ...
    payload = {"query": CREATE_LINEAR_ISSUE_MUTATION, "variables": variables}
...
def sync_github_to_linear():
    ...
    for issue in issues:
        ...
        if linear_issue:
            print(
                f"Tâche créée dans Linear: {linear_issue.get('title')} (URL: {linear_issue.get('url')})"
            )
        else:
